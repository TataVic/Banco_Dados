...
--PDuração no Projeto. 
ALTER TRIGGER Ex5L2 ON Projeto
AFTER UPDATE
AS
BEGIN
    IF UPDATE (PDuracao)
    BEGIN
        INSERT INTO Historico_Projeto (PNro,PNome, PDuracao, PCusto, DataAlteracao)
        SELECT 
			i.PNro, 
			i.PNome, 
			i.PDuracao,
    ...
    END
END;
UPDATE Projeto set PDuracao = '1000' where PNro = 10
select * from Historico_Projeto
